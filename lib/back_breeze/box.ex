defmodule BackBreeze.Box do
  ...
  defp render_and_calc(%{box: box} = acc, opts) do
    ...
    case render_path do
      :wrapped ->
        render_wrapped_content_container(acc, prev_id, rendered_child, opts)

      :plain_content ->
        render_plain_content_container(acc, prev_id, rendered_child)

      :passthrough_layer_map ->
        render_passthrough_layer_map_container(acc, prev_id, hd(children), rendered_child)
  ...
  defp render_children(
    ...
    %{
      ...
      fixed_layer_map: grid_fixed_layer_map
    } =
  ...
  defp render_children(%{box: %{children: children} = box} = acc, opts) when children != [] do
    ...
    flow_context = %{display: box.display, parent_width: parent_width}
        ...
        |> Enum.reduce({[], acc, 0}, fn child, {boxes, child_acc, used_extent} ->
          ...
          child_result =
            BenchProfile.measure({__MODULE__, :flow_child_render}, fn ->
              layout_only_child_result(child, opts) ||
      ...
      BenchProfile.measure({__MODULE__, :flow_join}, fn ->
        if BlockLayerMap.retain_for_combine?(box, retain_layer_map_context) do
          {width, height, layer_map, fixed_layer_map} =
            BlockLayerMap.compose(relative, box)
